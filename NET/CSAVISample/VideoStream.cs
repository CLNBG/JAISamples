...
namespace AviFile
{
	public class VideoStream : AviStream
	{
        ...
        private Avi.COMPVARS m_CompVars = new Avi.COMPVARS();
        private Avi.BITMAPINFOHEADER m_bmih = new Avi.BITMAPINFOHEADER();
        private Avi.BITMAPINFOHEADER8BPP m_bmih8bpp = new Avi.BITMAPINFOHEADER8BPP();
        private IntPtr m_lpbmih = IntPtr.Zero;
        private IntPtr m_lpCompressionState = IntPtr.Zero;
        ...
        public VideoStream(int aviFile, bool writeCompressed, double frameRate, int frameSize, int width, int height, PixelFormat format, IntPtr parentWindowHandle) {
			...
			this.firstFrame = 0;
            this.parentWindow = parentWindowHandle;
        ...
        public VideoStream(int aviFile, bool writeCompressed, double frameRate, Bitmap firstFrame, IntPtr parentWindowHandle) {
            this.parentWindow = parentWindowHandle;
        ...
        /// <param name="frameRate">Frames per second</param>
        /// <param name="firstFrame">Image to write into the stream as the first frame</param>
        public VideoStream(int aviFile, Avi.AVICOMPRESSOPTIONS compressOptions, double frameRate, Bitmap firstFrame, IntPtr parentWindowHandle)
        {
            this.parentWindow = parentWindowHandle;
		...
		public VideoStream(int aviFile, IntPtr aviStream){
            ...
            int size = Marshal.SizeOf(m_bmih);
            Avi.AVIStreamReadFormat(aviStream, 0, ref m_bmih, ref size);
            ...
            this.frameSize = m_bmih.biSizeImage;
            this.countBitsPerPixel = m_bmih.biBitCount;
        ...
        public override void Close()
        {
            ...
            if (m_CompVars.cbSize != 0)
                Avi.ICCompressorFree(m_CompVars);

            if (m_lpbmih != IntPtr.Zero)
                Marshal.FreeHGlobal(m_lpbmih);
        }
        ...
        private void Initialize(int aviFile, bool writeCompressed, double frameRate, Bitmap firstFrameBitmap) {
            ...
            this.pixelFormat = firstFrameBitmap.PixelFormat;
		...
		private void CreateCompressedStream(){
            ...
            {
                m_bmih8bpp.biSize = Marshal.SizeOf(m_bmih8bpp);
                m_bmih8bpp.biWidth = width;
                m_bmih8bpp.biHeight = height;
                m_bmih8bpp.biPlanes = 1;
                m_bmih8bpp.biBitCount = countBitsPerPixel;
                m_bmih8bpp.biSizeImage = frameSize;
                m_bmih8bpp.biClrUsed = 256;
                m_bmih8bpp.Palette = new byte[256 * 3];

                for (int i = 0; i < 256; i++)
                {
                    m_bmih8bpp.Palette[i*3] = (byte)i;
                    m_bmih8bpp.Palette[(i * 3) + 1] = (byte)i;
                    m_bmih8bpp.Palette[(i * 3) + 2] = (byte)i;
                }

                m_lpbmih = Marshal.AllocHGlobal(Marshal.SizeOf(m_bmih8bpp));
                Marshal.StructureToPtr(m_bmih8bpp, m_lpbmih, false);
            }
            else
            {
                m_bmih.biSize = Marshal.SizeOf(m_bmih);
                m_bmih.biWidth = width;
                m_bmih.biHeight = height;
                m_bmih.biPlanes = 1;
                m_bmih.biBitCount = countBitsPerPixel;
                m_bmih.biSizeImage = frameSize;
                m_lpbmih = Marshal.AllocHGlobal(Marshal.SizeOf(m_bmih));
                Marshal.StructureToPtr(m_bmih, m_lpbmih, false);
            }

            // Make the user select the codec
            m_CompVars.cbSize = Marshal.SizeOf(m_CompVars);
            if (Avi.ICCompressorChoose(parentWindow, (int)(Avi.ICMF_CHOOSE_KEYFRAME | Avi.ICMF_CHOOSE_DATARATE), m_lpbmih, IntPtr.Zero, m_CompVars, "Choose Video Encoder for the AVI file"))
            {
                // Sometimes not set!
                m_CompVars.lpbiIn = m_lpbmih;
                options.cbParms = (uint)m_CompVars.cbState;
                options.cbFormat = (uint) Marshal.SizeOf(m_bmih);  // critical!    
                // m_CompVars.lDataRate is in units of KBPS : KBPS = 1024*BPS     
                options.dwBytesPerSecond = (uint)(m_CompVars.lDataRate * 1024);
                options.dwFlags = Avi.AVICOMPRESSF_VALID | Avi.AVICOMPRESSF_KEYFRAMES | Avi.AVICOMPRESSF_DATARATE; // AVICOMPRESSF_VALID is critical!    
                options.dwInterleaveEvery = 0; // no interleaving...    
                options.dwKeyFrameEvery = (uint)m_CompVars.lKey;
                options.dwQuality = (uint)m_CompVars.lQ;
                options.fccHandler = (uint)m_CompVars.fccHandler; // critical!    
                options.fccType = (uint)m_CompVars.fccType; // critical!    
                options.lpFormat = m_lpbmih; // critical!    

               // allocate ram for state 
                m_lpCompressionState = Marshal.AllocHGlobal(m_CompVars.cbState);

                if (m_lpCompressionState != IntPtr.Zero)
                {
                    // reset the codec, very important!     
                    Avi.ICSendMessage(m_CompVars.hic, Avi.ICM_GETSTATE, m_lpCompressionState, m_CompVars.cbState);
                    Avi.ICSendMessage(m_CompVars.hic, Avi.ICM_SETSTATE, IntPtr.Zero, m_CompVars.cbState);
                    Avi.ICSendMessage(m_CompVars.hic, Avi.ICM_SETSTATE, m_lpCompressionState, m_CompVars.cbState);
                }
                ...
                if (result != 0)
                {
                    throw new Exception("Exception in AVIMakeCompressedStream: " + Avi.FormatAviErrorMessage(result));
                }

                SetFormat(compressedStream);
            }
            else
                throw new Exception("Unable to select AVI-file encoder (or user pressed the Cancel button)");
        }
		...
		private void SetFormat(IntPtr aviStream){
            if (this.PixelFormat == PixelFormat.Format8bppIndexed)
            {
                ...
                int structureSize = Marshal.SizeOf(bi) + Avi.PALETTE_SIZE;

                // Allocate memory for complete structure
                IntPtr buffer = Marshal.AllocHGlobal(structureSize);

                // Copy BITMAPINFOHEADER to the buffer
                Marshal.StructureToPtr(bi, buffer, false);

                // Create the Palette right after the BITMAPINFOHEADER structure
                unsafe
                {
                    byte* pData = (byte*)buffer;
                    pData += 40; // Point to right after the BITMAPINFOHEADER
                    for (int i = 0; i < 256; i++)
                    {
                        *pData = (byte)i; // Red
                        pData++;
                        ...
                        *pData = (byte)i; // Blue
                        pData++;
                        *pData = 0;       // Flags: PC_NOCOLLAPSE
                        pData++;
                    }
                }

                int result = Avi.AVIStreamSetFormat(aviStream, 0, buffer, structureSize);
                if (result != 0) { throw new Exception("Error in VideoStreamSetFormat: " + Avi.FormatAviErrorMessage(result)); }
            }
            else
            {
                Avi.BITMAPINFOHEADER bi = new Avi.BITMAPINFOHEADER();
		...
		public AviManager DecompressToNewFile(String fileName, bool recompress, out VideoStream newStream2, IntPtr parentWindowHandle){
			AviManager newFile = new AviManager(fileName, false, IntPtr.Zero);
			
			this.GetFrameOpen();
			
			Bitmap frame = GetBitmap(0);
            VideoStream newStream = newFile.AddVideoStream(recompress, frameRate, frame, parentWindowHandle);
			frame.Dispose();
		...
		public override void ExportStream(String fileName){
            ExportStream(fileName, IntPtr.Zero);
		}

        public void ExportStream(String fileName, IntPtr parentWindow)
        {
...
//            opts.lpFormat = IntPtr.Zero;
            ...
            Avi.AVISaveOptions(parentWindow, Avi.ICMF_CHOOSE_KEYFRAME | Avi.ICMF_CHOOSE_DATARATE, 1, ref streamPointer, ref opts);
