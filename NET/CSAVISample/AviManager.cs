...
namespace AviFile
{
	public class AviManager
	{
        ...
        /// <param name="parentWindow">Windows handle to be used as parent window when the Video Codec selection window is opened</param>
		public AviManager(String fileName, bool open, IntPtr parentWindow){
		...
		public VideoStream AddVideoStream(bool isCompressed, double frameRate, int frameSize, int width, int height, PixelFormat format){
            VideoStream stream = new VideoStream(aviFile, isCompressed, frameRate, frameSize, width, height, format, parentWindowHandle);
            streams.Add(stream);
			return stream;
		}
        ...
        /// <returns>VideoStream object for the new stream</returns>
        public VideoStream AddVideoStream(Avi.AVICOMPRESSOPTIONS compressOptions, double frameRate, Bitmap firstFrame, IntPtr parentWindowHandle)
        {
            VideoStream stream = new VideoStream(aviFile, compressOptions, frameRate, firstFrame, parentWindowHandle);
            streams.Add(stream);
            return stream;
        }
		...
		/// <param name="firstFrame">Image to write into the stream as the first frame</param>
		/// <returns>VideoStream object for the new stream</returns>
		public VideoStream AddVideoStream(bool isCompressed, double frameRate, Bitmap firstFrame, IntPtr parentWindowHandle){
			VideoStream stream = new VideoStream(aviFile, isCompressed, frameRate, firstFrame, parentWindowHandle);
            streams.Add(stream);
			return stream;
		}
        ...
        public void AddAudioStream(String waveFileName, int startAtFrameIndex) {
            AviManager audioManager = new AviManager(waveFileName, true, IntPtr.Zero);
        ...
        public AviManager CopyTo(String newFileName, float startAtSecond, float stopAtSecond) {
            AviManager newFile = new AviManager(newFileName, false, IntPtr.Zero);
            ...
            try {
                ...
                VideoStream newStream = newFile.AddVideoStream(false, videoStream.FrameRate, bmp, IntPtr.Zero);
        ...
        public static void MakeFileFromStream(String fileName, AviStream stream) {
            MakeFileFromStream(fileName, stream, IntPtr.Zero);
        }

        public static void MakeFileFromStream(String fileName, AviStream stream, IntPtr parentWindow)
        {
