...
HANDLE CreateAvi(LPCWSTR fn, int frameperiod, const WAVEFORMATEX *wfx);
...
HRESULT AddAviFrame(HANDLE avi, LPVOID pData, long lDataLength);

HRESULT AddAviFrame(HANDLE avi, HBITMAP hbm);

HRESULT AddAviAudio(HANDLE avi, void *dat, unsigned long numbytes);
...
HRESULT AddAviWav(HANDLE avi, LPCWSTR wav, DWORD flags);
...
HRESULT SetAviVideoCompression(HANDLE avi, HBITMAP hbm, AVICOMPRESSOPTIONS *opts, bool ShowDialog, HWND hparent);
